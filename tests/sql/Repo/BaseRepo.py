...
from .DBPool import PostgreSQLConnectionPool # type: ignore
...
class BaseRepo(ABC):
    ...
    def read_with_relations(self, relations: Optional[List[str]] = None, **kwargs) -> List[Any]:
        ...
        # 获取所有仓库的映射，用于加载关联对象
        repo_mapping = self._get_repo_mapping()
        
        # 为每个关系加载关联对象
        for relation_name in relations_to_load:
            relation = model_relations[relation_name]
            target_model_name = relation.to
            
            if target_model_name not in repo_mapping:
                continue
            
            target_repo = repo_mapping[target_model_name]
            
            # 根据关系类型加载关联对象
            if relation.relationship_type == "many_to_one":
                self._load_many_to_one_relations(instances, relation_name, relation, target_repo)
            elif relation.relationship_type == "one_to_many":
                self._load_one_to_many_relations(instances, relation_name, relation, target_repo)
        
        return instances

    def _load_many_to_one_relations(self, instances: List[Any], relation_name: str, 
                                   relation: Any, target_repo: Any) -> None:
        """加载多对一关系"""
        # 获取外键字段名
        fk_field = None
        for field_name, fk_def in self._model_class.get_foreign_keys().items():
            if fk_def.to == relation.to:
                fk_field = field_name
                break
        ...
        # 批量查询关联对象
        target_instances = target_repo.read(id__in=list(fk_values))
        
        # 创建ID到实例的映射
        target_map = {inst.id: inst for inst in target_instances}
        
        # 设置关联对象
        for instance in instances:
            fk_value = getattr(instance, fk_field)
            if fk_value is not None and fk_value in target_map:
                instance.set_related_object(relation_name, target_map[fk_value])

    def _load_one_to_many_relations(self, instances: List[Any], relation_name: str, 
                                   relation: Any, target_repo: Any) -> None:
        """加载一对多关系"""
        # 获取当前模型的主键字段
        pk_field = self._model_class.get_primary_key() or "id"
        ...
        for field_name, fk_def in target_repo._model_class.get_foreign_keys().items():
            if fk_def.to == self._model_class.__name__:
                target_fk_field = field_name
                break
        ...
        # 批量查询关联对象
        filter_kwargs = {f"{target_fk_field}__in": list(pk_values)}
        target_instances = target_repo.read(**filter_kwargs)
        
        # 按外键值分组
        grouped_targets = {}
        for target in target_instances:
            fk_value = getattr(target, target_fk_field)
            if fk_value not in grouped_targets:
                grouped_targets[fk_value] = []
            grouped_targets[fk_value].append(target)
        
        # 设置关联对象
        for instance in instances:
            pk_value = getattr(instance, pk_field)
            if pk_value in grouped_targets:
                instance.set_related_object(relation_name, grouped_targets[pk_value])

    def _get_repo_mapping(self) -> Dict[str, Any]:
        """获取所有仓库的映射"""
        from ..Repo import TasksRepo, TaskStepsRepo, ToolCallsRepo, MemoryLogRepo
        
        return {
            "Task": TasksRepo.TasksRepo(self._connection_pool),
            "TaskStep": TaskStepsRepo.TaskStepsRepo(self._connection_pool),
            "ToolCall": ToolCallsRepo.ToolCallsRepo(self._connection_pool),
            "MemoryLog": MemoryLogRepo.MemoryLogRepo(self._connection_pool)
        }
    ...
    def get_by_id_with_relations(self, id: int, relations: Optional[List[str]] = None) -> Optional[Any]:
        """
        根据ID获取单条记录并加载关联对象
        ...
        Returns:
            包含关联对象的模型实例或None
        """
        results = self.read_with_relations(relations=relations, id=id)
        return results[0] if results else None
