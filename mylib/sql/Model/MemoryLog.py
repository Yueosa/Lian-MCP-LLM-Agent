...
from .Enum import memory_log_role, memoty_log_memory_type
...
class MemoryLog(BaseModel):
    ...
    memory_type: memoty_log_memory_type = Field(..., description="记忆类型")
    importance: float = Field(default=0.5, description="重要性评分 (0-1) ")
    created_at: datetime = Field(default_factory=datetime.now, description="创建时间")
    
    class Config:
        """Pydantic 配置"""
        use_enum_values = False  # 保留 Enum 对象，便于序列化时控制
        json_encoders = {
            memory_log_role: lambda v: v.value,
            memoty_log_memory_type: lambda v: v.value,
